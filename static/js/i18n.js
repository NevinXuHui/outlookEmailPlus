(function () {
    ...
    const exactMap = {
        ...
        '全局搜索邮箱…': 'Search all mailboxes...',
        '全局搜索': 'Global Search',
        '所属分组': 'Group',
        '未找到匹配的邮箱': 'No matching mailboxes found',
        '搜索中…': 'Searching...',
        ...
        '已选择分组内所有 ${totalSelected} 个邮箱': 'Selected all ${totalSelected} accounts in the group',
        '已选择搜索结果中所有 ${totalSelected} 个邮箱': 'Selected all ${totalSelected} accounts in search results',
        '当前页暂无可选邮箱': 'No selectable accounts on this page',
        '全选失败，请重试': 'Failed to select all, please try again',
        '全选当前分组的所有邮箱（跨页）': 'Select all accounts in current group (cross-page)',
        '选择': 'Select',
        ...
        '正在取消刷新…': 'Cancelling refresh…',
        '当前没有进行中的刷新任务': 'No refresh task is currently running',
        '取消刷新失败': 'Failed to cancel refresh',
        '已请求取消刷新任务': 'Refresh cancel requested',
        '加载更多…': 'Loading more...',
        ...
        '刷新并发数必须在 1-20 之间': 'Refresh concurrency must be between 1 and 20',
        ...
        '文件导入': 'File Import',
        '点击选择或拖拽文件到此处': 'Click to select or drag files here',
        '支持多文件 · .txt / .csv · 与下方文本格式相同': 'Multiple files supported · .txt / .csv · same format as the text below',
        '准备导入…': 'Preparing import…',
        '开始导入…': 'Starting import…',
        '正在导入…': 'Importing…',
        '正在导入': 'Importing',
        ...
        '导入失败': 'Import failed',
        '导入进行中，请稍候': 'Import in progress, please wait',
        '部分文件已跳过': 'Some files were skipped',
        '未选择有效文件': 'No valid files selected',
        ...
        '邮箱----密码----client_id----refresh_token（或反序）': 'email----password----client_id----refresh_token (or reversed)',
        ...
        '添加': 'Add',
        '移动到分组': 'Move to Group',
        '确定要删除该分组吗？分组下的所有邮箱也将被删除，此操作不可恢复！': 'Delete this group? All accounts in the group will also be deleted. This action cannot be undone!',
        ...
        '选中数量较大，拉取可能耗时较长，确认继续？': 'A large number of accounts is selected. Fetching may take a long time. Continue?',
        '请先选择分组': 'Please select a group first',
        '⚠️ 异常': '⚠️ Anomalies',
        '🔎 批量检测': '🔎 Batch Detect',
        '获取账号列表失败': 'Failed to fetch account list',
        '当前分组没有可检测的账号': 'No accounts available to detect in the current group',
        '将批量刷新当前范围内 ${count} 个账号的 Token，用于快速筛选异常 Outlook。是否继续？': 'Refresh tokens for ${count} accounts in the current range to quickly filter abnormal Outlook accounts. Continue?',
        '批量刷新功能不可用': 'Batch refresh is unavailable',
        '批量检测失败': 'Batch detection failed',
        '检测完成，已切换到异常筛选': 'Detection completed. Switched to anomaly filter',
        '不移动（仅拉取）': 'Do not move (fetch only)',
        '请填写特定发件人后再选择目标分组': 'Enter a sender filter before choosing a target group',
        '请选择无匹配时要移动到的目标分组': 'Select a target group for unmatched accounts',
        '正在移动无匹配账号': 'Moving unmatched accounts',
        '命中发件人': 'Matched sender',
        '已移动': 'Moved',
        '待移动': 'Pending move',
        '部分移动失败': 'Some moves failed',
        '移动失败': 'Move failed',
        '移动分组失败': 'Failed to move group',
        '将拉取每个账号的收件箱 + 垃圾箱（各最近 15 封）': 'Fetch inbox + junk for each account (latest 15 each)',
        '特定发件人（可选）': 'Specific sender (optional)',
        '支持部分匹配（忽略大小写）。多个条件用逗号/空格/换行分隔，命中任一即保留。': 'Partial match (case-insensitive). Separate multiple patterns by comma/space/newline; any match keeps the account.',
        '无匹配时移动到分组': 'Move unmatched accounts to group',
        '仅对“拉取成功且未匹配到特定发件人”的账号执行移动；拉取失败的账号不会移动。': 'Only successfully fetched accounts without a matching sender are moved; failed fetches are not moved.',
        '开始拉取': 'Start Fetch',
        '不移动': 'Do not move',
        '命中发件人时移动到': 'Move matched to group',
        '无匹配时移动到': 'Move unmatched to group',
        '拉取失败时移动到': 'Move failed to group',
        '命中/无匹配分流需要填写特定发件人': 'Sender filter is required for matched/unmatched routing',
        '无匹配': 'Unmatched',
        '命中': 'Matched',
        '拉取成功且邮件中匹配到特定发件人的账号。': 'Accounts that fetched successfully and matched the sender filter.',
        '拉取成功但未匹配到特定发件人的账号。': 'Accounts that fetched successfully but did not match the sender filter.',
        'Token 失效、权限错误、限流等拉取失败的账号。不依赖发件人条件。': 'Accounts that failed to fetch (token/permission/rate-limit). Independent of sender filter.',
        '支持部分匹配（忽略大小写）。多个条件用逗号/空格/换行分隔，命中任一即算命中。命中/未命中分流需填写此项。': 'Partial match (case-insensitive). Separate patterns by comma/space/newline; any match counts. Required for matched/unmatched routing.',

        '每个邮箱拉取邮件数': 'Emails per account',
        '每邮箱': 'per mailbox',
        '每个账号在收件箱、垃圾箱分别拉取最近 N 封（默认 15，无上限）。': 'Fetch latest N emails from inbox and junk for each account (default 15, no upper limit).',
        '将拉取每个账号的收件箱 + 垃圾箱（各最近 N 封，可配置）': 'Fetch inbox + junk for each account (latest N each, configurable)',
        '并发数': 'Concurrency',
        '并发': 'concurrency',
        '服务端并发': 'server concurrency',
        '本波': 'wave',
        '同时拉取的账号数（默认 5，无上限）。由服务端线程池真正并行，不受浏览器连接数限制。过高可能触发上游限流或超时。': 'Accounts fetched at the same time (default 5, no upper limit). True server-side thread-pool parallelism, not limited by browser connections. Too high may trigger upstream rate limits or timeouts.',
        '收件箱 + 垃圾箱': 'Inbox + Junk'
    };
    ...
    const patterns = [
        ...
        { zh: /^共 (\d+) 条$/, en: 'Total $1' },
        { zh: /^请输入有效的页码 \(1-(\d+)\)$/, en: 'Please enter a valid page number (1-$1)' },
